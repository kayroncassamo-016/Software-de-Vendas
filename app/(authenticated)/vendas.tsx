...
export default function VendasList() {
  ...
  const [vendaSeleccionada, setVendaSeleccionada] = useState<Vendas | null>(
    null,
  );
  ...
  async function loadStats() {
    try {
      await Promise.all([loadVendas(), loadClientes(), loadFornecedores()]);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const VendaItem = ({ venda, onPress }: clienteItemProps) => {
    ...
    const cancelarVenda = async () => {
      try {
        ...
        setVendas((prev) =>
          prev.map((v) =>
            v.id === venda.id ? { ...v, estado: "CANCELADO" } : v,
          ),
        );
    ...
    return (
      <Swipeable
        ...
        onSwipeableOpen={(direction) => {
          if (direction === "left" && venda.estado === "RASCUNHO") {
            Alert.alert("Confirmar venda", "Queres confirmar esta venda?", [
              {
                text: "Cancelar",
                style: "cancel",
          ...
          if (direction === "right" && venda.estado === "RASCUNHO") {
            Alert.alert("Cancelar venda", "Queres cancelar esta venda?", [
              {
                text: "Não",
                style: "cancel",
                onPress: () => swipeableRef.current?.close(),
      ...
      >
        <TouchableOpacity style={styles.clienteCard} onPress={onPress}>
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <View>
            ...
            <Text style={styles.headerSub}>
              {filtrados?.length || 0} Vendas no catálogo
            </Text>
          </View>

          <TouchableOpacity
            onPress={() => AbrirVendasForm()}
            style={{ paddingTop: 10 }}
          >
